...
app.post('/v1/stock', checkToken, async (req, res ) => {
  const { googleSheets, auth, spreadsheetId } = await getAuthSheets()
  ...
  const StockRes = await StocksModel.create(dateSave)
  res.status(200).json({StockRes})
})
app.get('/v1/stock/:userId', checkToken, async (req, res ) => {
  const {userId} = req.params

  const dateValues = await StocksModel.find({user: userId})
...
app.get('/dados-actives', async (req, res)=> {
  const Codes = await axios.get(`https://brapi.dev/api/quote/list`)
...
//CRUD ACTIVES B3
app.post('/stocks', checkToken, async (req,res) => {
  const { userId } = req as any
  const { buyValue, amountBuy, dateBuy, codeName, category, type } = req.body as any
  const { googleSheets, auth, spreadsheetId } = await getAuthSheets()
  const getSTOCKs = await googleSheets.spreadsheets.values.get({
    auth,
    spreadsheetId,
  ...
  const response = await StocksModel.create(stock)

  res.json({response})
})
app.get('/stocks', checkToken, async (req, res)=> {
  const { userId } = req as any
  const { googleSheets, auth, spreadsheetId } = await getAuthSheets()
  const getSTOCKs = await googleSheets.spreadsheets.values.get({
    auth,
  ...
  const response = await StocksModel.find({ user: userId })
...
app.post('/active', checkToken, async (req, res) => {
  const { buyValue, amountBuy, dateBuy, codeName, category 
  } = req.body as any
  ...
  const data = newStocks.map(stock => {
    ...
    if(stock.buyQuant !== 0){
      ...
      return {
        ...
        UpDown: downUp,
        currentVariation: stock.currentVariation,//corrigir
